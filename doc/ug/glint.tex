...
\begin{figure}[htbp]
  ...
  \caption{Relationship between the time-steps in GLINT. The filled circles
  represent time-steps, the rectangles represent averaging/accumulation, and the arrows,
...
\item Global input fields must be supplied on a latitude-longitude
  ...
  Gaussian grids may be used. Non-rectangular grids (e.g. icosahedral grids) are not
...
hours. This is known as the \emph{forcing time-step}. 
\item \texttt{paramfile} an array of GLINT configuration filenames.
\end{itemize}
%
...
\item \texttt{precip} is the global precipitation rate,
  ...
  between rain, snow, etc.). Depending on the details of your climate
  model, and the mass-balance you select, it may be appropriate to
  supply precipitation minus evaporation instead.
...
daily basis (the \emph{Daily PDD scheme}). Both mass-balance schemes
calculate melt based on a sinusoidal cycle (annual and diurnal,
respectively). The amplitude and mean value of the cycle is determined
from the input temperature field: a spectral method is used to fit a
sinusoidal wave thought the supplied data. 
%
...
%%%% 
    ...
    \texttt{acab\_mode} & {\raggedright
      ...
      \begin{tabular}{lp{7cm}}
        ...
        3 & Hourly energy-balance model (from RAPID - not yet available) \\
        4 & Daily PDD mass-balance model\\
      \end{tabular}}\\
    ...
    default is equal to mass-balance time-step).  See below for full explanation of GLINT
    time-stepping. \\
    \hline
%%%% 
    \hline
    \multicolumn{2}{|l|}{\texttt{[GLINT output]}}\\
    \hline
    \multicolumn{2}{|p{0.98\textwidth}|}{Specifies output files in a
    similar way to the main GLIDE output section, \texttt{[CF
    output]}. A separate method of handling GLINT output is necessary
    because of the decoupling of timekeeping between GLINT and
    GLIDE. Specifying a \texttt{[GLINT output]} config section allows
    the output of instantaneous fields at intervals shorter than the
    GLIDE time-step. All the same values may be specified as for a \texttt{[CF
    output]} section. The possible fields are given in appendix \ref{ug.sec.varlist}}\\
    \hline
%%%% 
    \hline
    \multicolumn{2}{|l|}{\texttt{[GLINT maxmin-arng]}}\\
    \hline
    \multicolumn{2}{|p{0.98\textwidth}|}{Specifies that annual or
    diurnal temperature cycles used for mass-balance calculations
    should be obtained from the maximum and minimum values of the data,
    rather than by fitting a sinusoidal variation through the
    data. This section title, if present, must be in the top-level
    configuration file.}\\
    \hline  
    \end{supertabular}
\end{center}

\subsection{GLINT time-stepping}

By default, the model accepts input on each forcing time-step (as specified in 
